...
var exists = fs.exists; // eslint-disable-line
...
function load(settings, options, config, callback) {
  /** @type {{ loaded: any[] }} */ (config).loaded = [];
  // first load the root nodemon.json
  loadFile(options, config, utils.home, function (
    /** @type {any} */ options
  ) {
    // then load the user's local configuration file
    if (settings.configFile) {
      /** @type {{ configFile: string }} */ (options).configFile = path.resolve(
        settings.configFile
      );
    }
    loadFile(options, config, process.cwd(), function (
      /** @type {any} */ options
    ) {
      ...
      if (options.verbose) {
        /** @type {{ debug: boolean}} */ (utils).debug = true;
      }
      ...
      var ready = function (/** @type {any} */ options) {
 ...
 * @param {(options: object) => void} ready
 */
function normaliseRules(options, ready) {
  ...
  options.watch = options.watch === false ? false : rules.watch.rules;
...
function loadPackageJSON(config, ready) {
  ...
  return loadFile(
    ...
    config,
    dir,
    (/** @type {{ nodemonConfig: any; }} */ settings) => {
